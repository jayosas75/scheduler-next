...
> ### ▶ DO THIS NEXT
> Four of the five Security items are shipped (CVEs + password policy in `63f75c7`; login rate-limit in `ae68d96`, both pushed). The **email-enumeration fix is done but not yet committed** (working tree: register route, email.ts, its test). Order of operations:
> 1. **Commit & deploy** the working-tree changes so the enumeration fix goes live.
> 2. **Strip the `console.log('Invalid credentials')` in auth.ts** (~5 min) — the last Security checkbox, finishes "lock the doors."
