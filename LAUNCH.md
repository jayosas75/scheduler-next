...
- [ ] **Rate-limit the login path.** Register/forgot/reset are throttled; the credentials callback at [auth.ts:23](src/auth.ts#L23) `authorize()` is not. Add an IP-keyed cap (e.g. 10/min, 50/hr) inside `authorize`. *(30 min)*
