...
> ### ▶ DO THIS NEXT
> 🎉 **The whole P0 Security section is done.** The doors are locked: CVEs + password policy (`63f75c7`), login rate-limit (`ae68d96`), email-enumeration fix (`8812cac`), and the auth.ts log strip (working tree). Order of operations:
> 1. **Commit & deploy** the working-tree changes (auth.ts log strip) so prod is current.
> 2. Start **Reliability + recovery** — the next unchecked section. Lowest-effort first: **confirm Supabase auto-backups are on** (15 min, but it's a *decision* about your data tier — no real users should exist without it). Then **adopt `prisma migrate`** (stops silent schema drift) and **wire up Sentry** (so you learn about breakage before users complain).
...
- [x] **Stop leaking which emails are registered.** *(done 2026-06-10)* [register/route.ts](src/app/api/auth/register/route.ts) no longer returns the `400 "User already exists"` tell. New + existing emails now get a **byte-identical neutral 201** (`NEUTRAL_SUCCESS`), and no duplicate row is created. The real owner is nudged out-of-band via a new `sendAccountExists()` Resend email ([email.ts](src/lib/email.ts)) with sign-in + reset links. `bcrypt.hash` runs on both branches so response timing doesn't betray which path ran. Covered by [register-enumeration.test.ts](__tests__/register-enumeration.test.ts) (6 assertions: identical status/body, no "exists" leak, single row); full suite green (22).
- [x] **Strip `console.log('Invalid credentials')` from auth.ts.** *(done 2026-06-10)* Replaced with a structured `console.warn(\`[auth] failed login attempt from ip=${ip}\`)` — IP only, never the email, so prod logs stay useful for spotting credential-stuffing without echoing who was targeted. **✅ Security section complete.**
